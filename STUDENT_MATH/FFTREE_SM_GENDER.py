...
def disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class):
    ...
    print('<===> DATA IN DM AFTER CAT<===>')
    print(len(data_copy))
    
    y_pred_val = list(data_copy[data_copy[:,pro_index] == pri_val, :][:,-1])
    y_val = list(data_copy[data_copy[:,pro_index] == pri_val, :][:,-2])
    #print('===>' )
    #print(y_val)
    #print(y_pred_val)
    TP = 0
    ...
    try:
        ...
        TPR = 0
    print('===> DM RESULTS TP,FP,TN,FN;', TP,FP,TN,FN)
    print('===> DM RESULTS FPR,FNR,TPR;', FPR, FNR, TPR)
    return FPR, FNR, TPR
...
def fairness_attribute(data, categorical_features, pro_index, index_att, pri_val, unpri_val, pri_class, non_pri_class):
    ...
    for u in unique_values:
        ...
        mistreatment_pri = disparate_mistreatment(filter_data, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class)
        mistreatment_unpri = disparate_mistreatment(filter_data, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class)
    ...
    for i in range(len(predictors)):
        if np.unique(data[:,i][~pd.isna(data[:,i])]).tolist() != []:
            if i != pro_index and predictors[i] == True:
                ...
                DIs.append(DI)
                DMdata_pri = disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class)
                DMdata_unpri = disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class)  
