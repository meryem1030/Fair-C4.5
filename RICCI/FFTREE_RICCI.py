...
def devide_data(data):
    ...
    for c in range(len(a)): #5
        if int(list(a.values())[c]) > 0: #6
            for i in range(10): #6.2
                part[i] = part[i]._append(data[data.iloc[:,-1] == list(a.keys())[c]].iloc[int(list(a.values())[c])*i:int(list(a.values())[c])*(i+1),:])
    ...
    if len(remaining_cases) <= 10:
        for r in range(len(remaining_cases)):
            part[r] = part[r]._append(remaining_cases[r], ignore_index = True)

    else:#if len(remaining_cases) > 10:
        for r in range(len(part)):
            part[r] = part[r]._append(remaining_cases[r], ignore_index = True)
        for i in range(len(remaining_cases) - len(part)):
            part[i] = part[i]._append(remaining_cases[i + 10], ignore_index = True)
    return part
...
def disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, copy_val, pri_class, non_pri_class):
    ### START DEFAULT PROCEDURE ###
    data_copy = data.copy()
    weights = np.ones(len(data))
    distribution = Counter() #Find class distribution in each training sets
    class_attribute = data[:,-1] #Find class attribute values in each training set
    for value in np.unique(class_attribute):
        distribution[value] = weights[class_attribute == value].sum() #Find total numbers in the distribution
    majority_value = mode(list(distribution.values())) #Find majority value via using mode
    #print(majority_value)
    for key, value in distribution.items():
        if value == majority_value:
            majority_label = key #Find the key/majority class label
    ...
    y = data_copy[:,-1]
    col = np.array(y_pred)
    data_copy = np.column_stack((data_copy,col))
    
    if pro_index in categorical_features: #transform in binary case if the index in categorical list
        unique_values = np.unique(data_copy[:,pro_index][~pd.isna(data_copy[:,pro_index])])
        data_copy[(pd.notna(data_copy[:,pro_index]) & (data_copy[:,pro_index] != pri_val)), pro_index] = unpri_val
    else:# 1 is prival, 0 is unpri_val after changing the continues values to the binary
        att_threshold = gain_ratio_numeric(data_copy, pro_index, weights)[2]
        data_copy[pd.notna(data_copy[:,pro_index]), pro_index] = np.where(data_copy[pd.notna(data_copy[:,pro_index]), pro_index] > att_threshold, pri_val, unpri_val)
    
    y_pred_val = list(data_copy[data_copy[:,pro_index] == copy_val, :][:,-1])
    y_val = list(data_copy[data_copy[:,pro_index] == copy_val, :][:,-2])
    #print('===>' )
    #print(y_val)
    #print(y_pred_val)
    TP = 0
    ...
    for i in range(len(y_pred_val)): 
        if y_val[i]==y_pred_val[i]== pri_class:
            TP += 1
        if y_pred_val[i]== pri_class and y_val[i]!=y_pred_val[i]:
            FP += 1
        if y_val[i]==y_pred_val[i]== non_pri_class:
            TN += 1
        if y_pred_val[i]== non_pri_class and y_val[i]!=y_pred_val[i]:
            FN += 1
    try:
        FPR = FP/(FP+TN)
...
def split_DI_CV(data, pro_index, categorical_features, pri_val, unpri_val, pri_class):
    ...
    return pri_data, unpri_data, DI_DATA       
...
def fairness_attribute(data, categorical_features, pro_index, index_att, pri_val, unpri_val, pri_class, non_pri_class):
    weights = np.ones(len(data))
    data_original = data.copy()
    unique_list = []
    ...
    unpri_val_copy = unpri_val
    pri_val_copy = pri_val
    DI_results = []
    DM_FPR = []
    DM_FNR = []
    DM_TPR = []
    for u in unique_values:
        ...
        mistreatment_pri = disparate_mistreatment(filter_data, pro_index, categorical_features, pri_val, unpri_val, pri_val_copy, pri_class, non_pri_class)
        mistreatment_unpri = disparate_mistreatment(filter_data, pro_index, categorical_features, pri_val, unpri_val, unpri_val_copy, pri_class, non_pri_class)
        FPR = abs(mistreatment_pri[0] - mistreatment_unpri[0])
        FNR = abs(mistreatment_pri[1] - mistreatment_unpri[1])
        TPR = abs(mistreatment_pri[2] - mistreatment_unpri[2])
    ...
    return sum(DI_results), sum(DM_FPR), sum(DM_FNR), sum(DM_TPR)
...
def fairness_diffs(data, categorical_features, available_attributes, pro_index, pri_val, unpri_val, pri_class, non_pri_class):
    DIs = []
    DMs_FPR = []
    DMs_FNR = []
    DMs_TPR = []
    ...
    for i in range(len(predictors)):
        if np.unique(data[:,i][~pd.isna(data[:,i])]).tolist() != []:
            if i != pro_index and predictors[i] == True:
                #print('index:', i)
                ind.append(i)
                f_attribute = fairness_attribute(data, categorical_features, pro_index, i, pri_val, unpri_val, pri_class, non_pri_class)
                f_data = split_DI_CV(data, pro_index, categorical_features, pri_val, unpri_val, pri_class)
                DI = f_attribute[0] - f_data[2]
                DIs.append(DI)
                DMdata_pri = disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, pri_val_copy, pri_class, non_pri_class)
                DMdata_unpri = disparate_mistreatment(data, pro_index, categorical_features, pri_val, unpri_val, unpri_val_copy, pri_class, non_pri_class)  
                FPR_data = abs(DMdata_pri[0] - DMdata_unpri[0])
                FNR_data = abs(DMdata_pri[1] - DMdata_unpri[1])
                TPR_data = abs(DMdata_pri[2] - DMdata_unpri[2])
                FPR = f_attribute[1] - FPR_data
                DMs_FPR.append(FPR)
                FNR = f_attribute[2] - FNR_data
                DMs_FNR.append(FNR)
                TPR = f_attribute[3] - TPR_data
                DMs_TPR.append(TPR)
            #if i == pro_index and predictors[i] == True:
            ...
            FNR_data = -1
            TPR_data = -1
            DMs_FPR.append(FPR_data)
            DMs_FNR.append(FNR_data)
            DMs_TPR.append(TPR_data)
            
    return ind, DIs, DMs_FPR, DMs_FNR, DMs_TPR
...
def search_best_attribute(data, available_attributes, categorical_features, pro_index, weights):
    available_attributes_copy = available_attributes.copy()
    available_attributes_copy[pro_index] = False
    predictors = available_attributes_copy
...
def numpy_table(data, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, non_pri_class, weights):
    
    if available_attributes.count(True) >= 1:
        g = search_best_attribute(data, available_attributes, categorical_features, pro_index, weights)
        #d = Treatment_diffs(data, categorical_features, available_attributes, pro_index, pri_val, unpri_val, pri_class)
        f = fairness_diffs(data, categorical_features, available_attributes, pro_index, pri_val, unpri_val, pri_class, non_pri_class)
            ...
            for i in range(len(g[0])):
                ...
                for j in range(len(f[0])):
                    ...
                    if np_table[i,0] == f[0][j]:
                        np_table[i,1] = abs(f[1][j]-1)
                        np_table[i,2] = g[1][i]
                        #np_table[i,3] = abs(d[1][j]-0)
                        np_table[i,3] = abs(f[2][j]-0)
                        np_table[i,4] = abs(f[3][j]-0)
                        np_table[i,5] = abs(f[4][j]-0)
        else:
            np_table = np.array([])
    #print('===> np_table:,')
    #print(np_table)
...
def ff_tree(np_table):
    ...
    if len(np_table) == 0:
        ...
        for i in range(len(new_table)):
            if new_table[i,1] <= th and  new_table[i,3] <= th and new_table[i,4] <= th and new_table[i,5] <= th:
                candidates.append(i)
        ...
        if candidates != []:
            ...
            selection = []
            for j in candidates:
                obj_try = new_table[j, 2]
                selection_idx.append(j)
                selection.append(obj_try)
            print('selection:', selection)

            store = []
            find_max = max(selection) #since we used the ranking according to distance:rank 1 is the best att that close to 1(high acc)
            for k in range(len(selection)):
                if selection[k] == find_max:
...
def build_decision_nodes(metadata, data, df, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, non_pri_class, tree=None, parent_majority=None, weights=None):#instead of df use list of numeric attributes
    ...
    #print('\n\n===>data',data)
    np_table = numpy_table(data, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, non_pri_class, weights)
    #print('NP_TABLE IN BUILD TREE:', np_table)
    index = ff_tree(np_table)#math_model(data, np_table)
    #index = int(index)
    ...
    if valid < 2:
        ...
        if is_numeric_dtype(df.iloc[:,index]):
            ...
            branch_data = data[partition | missing]
            child = build_decision_nodes(metadata, branch_data, df, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, non_pri_class, node, majority, updated_weights)
            ...
            for idx, value in enumerate(metadata[index]):
                ...
                child = build_decision_nodes(metadata, branch_data, df, pro_index, categorical_features, available_attributes.copy(), pri_val, unpri_val, pri_class, non_pri_class, node, majority, updated_weights)
...
def consistency_test(data, test, pro_index, categorical_features, pri_val, unpri_val, pri_class,user_defined_threshold=None):
    ...
    return (consistency_pri + consistency_unpri)/2
...
def run(metadata, data, df, part, pro_index, categorical_features, pri_val, unpri_val, pri_class, non_pri_class):
    ...
    for i in range(len(part)): 
        ...
        tree = build_decision_nodes(metadata, train, df, pro_index, categorical_features, available_attributes, pri_val, unpri_val, pri_class, non_pri_class, tree=None, parent_majority=None, weights=None)
        print("\n\n===> Printing Tree\n")
        tree.to_text()
        y_pred = []
        correct = 0
